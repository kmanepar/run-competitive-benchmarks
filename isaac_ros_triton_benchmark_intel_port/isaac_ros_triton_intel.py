...
"""Performance test for Triton Node (DOPE model) on Intel via OpenVINO."""
import os, time
...
def launch_setup(container_prefix, container_sigterm_timeout):
    ...
    ns = TestTritonIntel.generate_namespace()
    nodes = [
...
def generate_test_description():
    return TestTritonIntel.generate_test_description_with_nsys(launch_setup)

class TestTritonIntel(ROS2BenchmarkTest):
    config = ROS2BenchmarkConfig(
        benchmark_name='Isaac ROS TritonNode (DOPE) Benchmark (Intel OpenVINO)',
