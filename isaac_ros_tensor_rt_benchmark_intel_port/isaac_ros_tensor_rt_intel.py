...
"""Performance test for TensorRT Node (DOPE model) on Intel via OpenVINO."""
import os, time
...
def launch_setup(container_prefix, container_sigterm_timeout):
    ...
    ns = TestTensorRtIntel.generate_namespace()
    nodes = [
        ...
        ComposableNode(name='DopeOpenVINONode', namespace=ns, package='isaac_ros_benchmark',
            ...
            parameters=[{'model_path': model_path, 'score_threshold': 0.3, 'object_name': 'ketchup',
                         'network_width': 640, 'network_height': 480, 'openvino_device': 'CPU',
...
def generate_test_description():
    return TestTensorRtIntel.generate_test_description_with_nsys(launch_setup)

class TestTensorRtIntel(ROS2BenchmarkTest):
    config = ROS2BenchmarkConfig(
        benchmark_name='Isaac ROS TensorRTNode (DOPE) Benchmark (Intel OpenVINO)',
    ...
    OV_INIT_WAIT_SEC = 3
    def pre_benchmark_hook(self): time.sleep(self.OV_INIT_WAIT_SEC)
